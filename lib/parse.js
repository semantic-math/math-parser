...
class Parser {
    ...
    implicitMul() {
        ...
        return factors.length > 1
            ? build.applyNode('mul', factors, null, {implicit: true})
            : factors[0];
    }
    ...
    factor() {
        ...
        if (matches(token, '#') || isIdentifierToken(token)) {
            ...
            if (matches(this.currentToken(), '(')) {
                ...
                token = this.consume(')');
                base = build.applyNode(node, args, {start, end: token.end})
            ...
            this.consume(token.value);
            base = build.numberNode(token.value, start, token.end);
        } else if (matches(token, '(')) {
            this.consume('(');
            base = this.expression();
            token = this.consume(')');
            if (base.type === 'Number' || isSymbol(base)) {
                base = build.parensNode(base, start, token.end);
            }
        } else if (matches(token, '|')) {
            this.consume('|');
            base = this.expression();
            ...
            base = build.applyNode('abs', [base], {start, end: token.end})
        }
        ...
        if (matches(this.currentToken(), '^')) {
            ...
            factor = build.applyNode('pow', [base, exp], loc);
        }
        ...
        signs.forEach((sign) => {
            ...
            if (sign.value === '+') {
                factor = build.applyNode('pos', [factor], loc);
            } else {
                factor = build.applyNode('neg', [factor], loc);
            }
        });
    ...
    identifier() {
        ...
        const name = token.value
        const result = build.identifierNode(name, start, token.end)
        this.consume(name)
        ...
        if (matches(token, '_')) {
           ...
           if (isNumberToken(token)) {
                result.subscript =
                    build.numberNode(token.value, token.start, token.end)
                this.consume(token.value)
            } else if (isIdentifierToken(token)) {
                result.subscript =
                    build.identifierNode(token.value, token.start, token.end)
                result.loc.end = token.end
                this.consume(token.value)
            } else {
