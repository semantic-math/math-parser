...
class Parser {
    ...
    factor() {
        ...
        if (matches(token, '#') || isIdentifierToken(token)) {
            ...
            // e.g. dy/dx
            else if (node.name.match(r1) != null && matches(this.currentToken(), '/')) {
                ...
                if  (token.value.match(r2) != null) {
                    ...
                    if (matches(this.currentToken() , '(')) {
                        ...
                        token = this.consume(')')
                        // d/dx (x^2)
                        base = build.apply('diff', [args[0], secondVar])
                    } else {
                        // dy/dx
            ...
            // Leibniz notation (n-ary diff)
            // e.g. d^2/dxdy(x^2 + y^2)
            else if (node.name.match(r1) != null && matches(this.currentToken(), '^')){
                this.consume('^')
                token = this.currentToken()
                const order = token.value
                this.consume(token.value)
                if (matches(this.currentToken(), '/')) {
                    this.consume('/')
                    const denom = this.currentToken().value
                    const variables = denom.split('d').slice(1)
                    this.consume(denom)
                    ...
                    if (matches(this.currentToken() , '(')) {
                        ...
                        base = build.apply('diff', [args, variables])
                    } else {
                        throw new Error('derivative expects an arg')
                    }
                } else {
                    base = node
                }
