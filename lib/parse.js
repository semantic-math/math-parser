...
class Parser {
    ...
    implicitMul() {
        ...
        if (query.isIdentifier(factor) && !isWellKnown(factor)) {
            factors = factor.name.split('').map(build.identifier)
            if (factor.subscript) {
                factors[factors.length - 1].subscript = factor.subscript
            }
        } else if (query.isImplicitMul(factor) && factor.wasSplit) {
            factors.push(...factor.args)
        } else {
            factors.push(factor)
        }
        ...
        while (true) {
            ...
            if (matches(token, '(') || matches(token, '#') || isIdentifierToken(token) || isNumberToken(token)) {
                ...
                if (factor) {
                    if (query.isIdentifier(factor) && !isWellKnown(factor)) {
                        factors.push(...factor.name.split('').map(build.identifier))
                        if (factor.subscript) {
                            factors[factors.length - 1].subscript = factor.subscript
                        }
                    } else if (query.isImplicitMul(factor) && factor.wasSplit) {
    ...
    factor() {
        ...
        if (matches(token, '#') || isIdentifierToken(token)) {
            ...
            if (matches(this.currentToken(), '(')) {
                ...
                if (query.isIdentifier(node) && !isWellKnown(node) && node.name.length > 1) {
                    const factors = node.name.split('').map(build.identifier)
                    if (node.subscript) {
                        factors[factors.length - 1].subscript = node.subscript
                    }
                    base = build.implicitMul(...factors)
                    base.wasSplit = true
                } else {
        ...
        if (matches(this.currentToken(), '^')) {
            this.consume('^')
            exp = this.factor()
            if (query.isImplicitMul(base) && base.wasSplit) {
                const lastFactor = factor.args[factor.args.length - 1]
                factor.args[factor.args.length - 1] =
                    build.applyNode('pow', [lastFactor, exp])
            } else {
    ...
    identifier() {
        ...
        const result = build.identifier(name)
        this.consume(name)
        ...
        if (matches(token, '_')) {
            ...
            if (isNumberToken(token)) {
                result.subscript = build.number(token.value)
                this.consume(token.value)
            } else if (isIdentifierToken(token)) {
                result.subscript = build.identifier(token.value)
                this.consume(token.value)
            } else {
