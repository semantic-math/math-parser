...
class Parser {
    ...
    factor() {
        ...
        if (matches(token, '#') || isIdentifierToken(token)) {
            ...
            if (matches(this.currentToken(), '(')) {
                ...
                token = this.consume(')');
                base = nodes.applyNode(node, args, {start, end: token.end})
    ...
    identifier() {
        ...
        const name = token.value
        const result = nodes.identifierNode(name, start, token.end)
        this.consume(name)
        ...
        if (token && token.value === '_') {
           ...
           if (isNumberToken(token)) {
                result.subscript =
                    nodes.numberNode(token.value, token.start, token.end)
                this.consume(token.value)
            } else if (isIdentifierToken(token)) {
                result.subscript =
                    nodes.identifierNode(token.value, token.start, token.end)
