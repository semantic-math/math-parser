        ...
        if (matches(token, '#') || isIdentifierToken(token)) {
            ...
            if (matches(this.currentToken(), '(')) {
                ...
                if (node.name === 'nthRoot') {
                    ...
                    switch (args.length) {
                    case 2:
                        base = build.apply('int', [args[0], args[1]])
                        break;
                    case 3:
                        base = build.apply('int', [args[0], args[1], args[2]])
                        break;
                    case 4:
                        base = build.apply('int', [args[0], args[1], args[2], args[3]])
                        break;
                    default:
                        throw new Error("integral takes between 2 - 4 args")
                    }
                } else {
