...
const isNeg = (node) => {
    return node && node.type === 'Apply' && node.op === 'neg';
}

const isPow = (node) => {
    return node && node.type === 'Apply' && node.op === 'pow';
}

const isMul = (node) => {
    return node && node.type === 'Apply' && node.op === 'mul';
}

const isDiv = (node) => {
    return node && node.type === 'Apply' && node.op === 'div';
}
...
const printApply = (node, parent) => {
    ...
    if (op === 'add') {
        ...
        const result = node.implicit
            ...
            : args.map(arg => print(arg, node)).join(` * `);
        return isMul(parent) || isPow(parent) || isDiv(parent)
            ? `(${result})`
            : result;
    } else if (op === 'div') {
        let result = '';
