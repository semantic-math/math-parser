...
export default function evaluate(node) {
    switch (node.type) {
        ...
        case 'Apply': {
            ...
            } else if (Math[op].length === args.length) {
                return Math[op].apply(null, args.map(evaluate));
            } else if (Math[op].length !== args.length) {
                throw new Error(`'${op}' takes ${Math[op].length} parameters, ${args.length} was provided`);
            } else {
