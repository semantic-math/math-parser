...
const applyToTex = (node, parent) => {
    ...
    if (op === 'add') {
        ...
        if (node.args.length == 2) {
            ...
            result = `\\int ${toTex(input, node)} ${toTex(variable, node)}`
        } else if (node.args.length == 3) {
            const [input, variable, domain] = node.args
            result = `\\int_${toTex(domain, node)} ${toTex(input, node)} ${toTex(variable, node)}`
        } else if (node.args.length == 4) {
            const [input, variable, upper, lower] = node.args
            result = `\\int_{${toTex(upper, node)}}^{${toTex(lower, node)}} ${toTex(input, node)} ${toTex(variable, node)}`
        }
        return result
